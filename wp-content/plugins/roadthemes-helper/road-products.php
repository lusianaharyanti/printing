...
class RoadProducts {
	...
	public static function sale_products_r( $atts ) { 
		...
		$enable_time='';
		if ($atts["enable_time"] == true) {
			$enable_time = 'show-time';
		} 
		
		$shortcode = new WC_Shortcode_Products( $atts, 'sale_products' );

		return '<div class="sale_product '.$style.' '.$enable_time.'">'.$shortcode->get_content().'</div>';
	}
	...
	vc_map( array(
		...
		'params' => array_merge(
			array(
				array(
					'type' => 'checkbox',
					'heading' => __( 'Enable Time Sale', 'devita' ),
					'param_name' => 'enable_time',
					'value' => true,
					'save_always' => true,  
				),
				array(
